...
function Accordion(e) {
    ...
    e = $.DOM;
    $.__SelectedItem = null;
    $.__Effect = e.getAttribute("effect") || "none";
    $.__HeaderClass = e.getAttribute("headerclass") || "header";
    $.__SelectedClass = e.getAttribute("selectedclass") || "selected";
    $.__ItemClass = e.getAttribute("itemclass") || "item";
    ...
    for (var i = 0; i < l; i++) {
        ...
        if (v.tagName == "DIV") {
            ...
            v.Classes.Add($.__ItemClass);
            v.Header.Classes.Add($.__HeaderClass);
            v.__Effect = new Effect(v, $.__Effect, 1, 0, Speed.Normal, Smoothness.Smoothest);
        }
    }
    ...
    switch (e.ModificationType) {
        case ModificationType.Add:
            for (i = 0; i < l; i++) {
                ...
                v.ClassName(this.__ItemClass).Header.ClassName(this.__HeaderClass);
                v.__Effect = new Effect(v, this.__Effect, 1, 0, Speed.Fastest, Smoothness.Normal);
            }
            break;
        ...
        case ModificationType.Update:
            this.ReSnapTos(Array.From(this.DOM.children));
            o = this.Items;
            l = o.lenth;
            for (i = 0; i < l; i++) {
                v = o[i];
...
Accordion.prototype.HeaderClass = function (c) {
    if (c === undefined) {
        return this.__HeaderClass;
    }

    this.__HeaderClass = c;
...
Accordion.prototype.SelectedClass = function (c) {
    if (c === undefined) {
        return this.__SelectedClass;
    }

    this.__SelectedClass = c;
    if (this.__SelectedItem) {
        this.__SelectedItem.ClassName(c);
    }
    return this;
}
...
Accordion.prototype.SelectedItem = function (i) {
    ...
    if (this.__SelectedItem) {
        this.__SelectedItem.Header.Classes.Remove(this.__SelectedClass);
        this.__SelectedItem.__Effect.Reverse();
    }

    this.__SelectedItem = i;
    if (i !== null) {
        this.__SelectedItem.__Effect.Play();
        this.__SelectedItem.Header.Classes.Add(this.__SelectedClass);
    }
...
Accordion.prototype.Effect = function (e) {
    ...
    for (i = 0; i < l; i++) {
        v = a[i];
        v.__Effect = new Effect(v, e, 1, 0, Speed.Fastest, Smoothness.Normal);
    }
...
AccordionItem.prototype.Selected = function (s) {
    ...
    if (s === undefined) {
        return p.__SelectedItem == this;
    }

    if (s === true) {
        if (p.__SelectedItem != this) {
            p.SelectedItem(this);
        }
...
function DatePicker(e) {
    ...
    e = $.DOM;
    $.__Display = new Div().Position("absolute").Collapsed(true);
    ...
    $.__Display.AddElement(t);
    $.__PadClass = e.getAttribute("padclass") || "pad";
    $.__PadDays = (e.getAttribute("pad") || "false").ToBoolean();
    $.__Selected = e.getAttribute("selectedclass") || "selected";
    $.__Effect = new Effect($.__Display, e.getAttribute("effect") || "none", 1, 0, Speed.Fastest, Smoothness.Normal);
    $.__CS = null;
    ...
    var prev = new Anchor().Href("javascript: Null();").Text("<");
    var next = new Anchor().Href("javascript: Null();").Text(">");
    ...
    b = [];
    d = new Delegate($.__DateSelected, $, [Delegate.Event]);
    for (i = 0; i < 6; i++) {
        a = [new Cell(),
             new Cell(),
             new Cell(),
    ...
    prev.OnClick.Add(new Delegate($.__ChangeMonth, $, [-1]));
    next.OnClick.Add(new Delegate($.__ChangeMonth, $, [1]));
    ...
    Page.AddElement($.__Display);
    $.Refresh();
};

// private event handlers

DatePicker.prototype.__ChangeMonth = function (c) {
    var m = this.__M + c;
    if (m == 0) {
        m = 12;
        this.__Y--;
...
DatePicker.prototype.__DateSelected = function (e) {
    ...
    this.__CS = s;
    s.ClassName(this.__Selected);
    this.Text(new DateTime(this.__Y, this.__M, this.__D));
    this.Focus();
    this.Hide(true);
};

DatePicker.prototype.Effect = function (e) {
    this.__Effect = new Effect(this.__Display, e, 1, 0, Speed.Fastest, Smoothness.Normal);
    return this;
}

DatePicker.prototype.PadDays = function (p) {
    if (p === undefined) {
        return this.__PadDays;
    }
...
DatePicker.prototype.Show = function () {
    ...
    this.__Display.X(this.X()).Y(this.Y() + this.LayoutHeight());
    this.__Effect.Play();
    return this;
};

DatePicker.prototype.Hide = function (f) {
    if ((this.IsFocused || this.__Display.IsMouseOver) && !f) {
        return;
    }

    if (this.__Display.Collapsed()) { return; }
    this.__Forced = f;
    this.__Effect.Reverse();
    return this;
...
DatePicker.prototype.Refresh = function () {
    this.__Display.DOM.firstChild.$.Head.Rows[0].Cells[1].Text(DateTime.Months.GetName(this.__M) + " " + this.__Y);
    var tb = this.__Display.DOM.firstChild.$.Body;
    var r = 1;
    ...
    for (i = 0; i < dow; i++) {
        if (this.__PadDays) {
            c.Cells[i].Text(cD.Date() + i).ClassName(this.__PadClass).Tag = -1;
        }
        else {
            c.Cells[i].Text("").ClassName(this.__PadClass).Tag = -1;
        }
    }
    ...
    for (j = r; j < 8; ) {
        ...
        c.Collapsed(false);
        for (; i < 7; i++) {
            c.Cells[i].Text(e).ClassName(this.__D == e ? this.__Selected : "").Tag = e;
            if (this.__D == e) {
                this.__CS = c.Cells[i];
            }
            e++;
    ...
    while (j < 7) {
        ...
        if (i == 0) {
            ...
            for (; i < 7; i++, e++) {
                if (this.__PadDays) {
                    c.Cells[i].Text(e).ClassName(this.__PadClass).Tag = -1;
                }
                else {
                    c.Cells[i].Text("").ClassName(this.__PadClass).Tag = -1;
                }
...
function Dialog(e) {
    ...
    var $ = this;
    e = $.DOM;
    $.__Effect1 = new Effect(Dialog.Filter, e.getAttribute("effect") || "none", Dialog.Opacity, 0, Speed.Faster, Smoothness.Smoother);
    $.__Effect2 = new Effect(Dialog.Filter.T, e.getAttribute("effect") || "none", 1, 0, Speed.Faster, Smoothness.Smoother);
...
Dialog.Color = "black",
Dialog.Opacity = .5,
Dialog.ZIndex = NinJaVisuals.ZIndex++,
Dialog.Filter = null;

Dialog.RemoveCurrent = function (data) {
    Dialog.Filter.T.Rows[0].Cells[0].ReSnapTo(Dialog.CurrentDialog);
...
Dialog.Hide = function (data) {
    ...
    if (d) {
        ...
        if (d.OnDialogClose.Fire(new EventArgs(d, data)) != false) {
            d.__Effect1.Reverse(Dialog.RemoveCurrent);
            d.__Effect2.Reverse();
            Dialog.CurrentDialog = null;
        }
    }
...
Dialog.prototype.Show = function () {
    ...
    Dialog.Dialogs.ReSnapTo(this);
    Dialog.Filter.T.Rows[0].Cells[0].AddElement(this);
    this.__Effect1.Play();
    this.__Effect2.Play();
    return this;
...
Type.FormTable = Type.$++;
Table.Extensions.Add(new Extension("form", FormTable));
...
//-------------------------------------------
//- Makes use of the <colgroup> tag that contains <col> children. The <col> children refer to sequential
//- columns and applies their attributes to each column with colspan is 1.
//-------------------------------------------
//- Declarative attributes
...
function FormTable(e) {
    ...
    var v, l;
    e = this.DOM;
    $.__HighlightClass = e.getAttribute("highlight") || "";

    if ($.__HighlightClass) {
        var inputs = Array.From($.ElementsByTagName("input"))
        ...
        for (var i = 0; i < l; i++) {
            v = inputs[i];
            v.OnFocus.Add(new Delegate($.__Highlight, $, [v, true]));
            v.OnBlur.Add(new Delegate($.__Highlight, $, [v, false]));
        }
    }

    $.ColumnGroups = new ObservableArray($);
    var o = Array.From(e.children);
    l = o.length;

    for (var i = 0; i < l; i++) {
        v = o[i];
        if (v.tagName == "COLGROUP") {
            $.ColumnGroups.Add(new ColumnGroup(v));
        }
    }
...
function ColumnGroup(e) {
    var $ = this;
    $.FormColumns = new ObservableArray($);
    $.FormRow = null;
    var a = e.children;
    var l = a.length;
    for (var i = 0; i < l; i++) {
        $.FormColumns.Add(new FormColumn(a[i]));
    }
}

FormTable.prototype.__ApplyStyling = function () {
        ...
        for (j = 0; j < h; j++) {
            ...
            for (n = 0; n < m; n++) {
                ...
                if (o.colSpan == 1) {
                    p = k.FormColumns[n];
                    if (p) {
                        o.setAttribute("valign", p.__VAlign);
                        o.setAttribute("align", p.__HAlign);
                        o.setAttribute("width", p.__Width);
                        o.setAttribute("style", p.__Style);
                        l[n].ClassName(p.__Class);
                    }
                }
            }
        }
...
FormTable.prototype.__Highlight = function (x, v) {
    var r = x.DOM;
    ...
    if (v === true) {
        if (!r.Classes.Contains(this.__HighlightClass)) {
            r.Classes.Add(this.__HighlightClass);
        }
    }
    else {
        r.Classes.Remove(this.__HighlightClass);
    }
};

function FormColumn(e) {
    var $ = this;
    $.__VAlign = e.getAttribute("valign") || "";
    $.__HAlign = e.getAttribute("align") || "";
    $.__Style = e.getAttribute("style") || "";
    $.__Class = e.getAttribute("class") || "";
    $.__Width = e.getAttribute("width") || "";

    $.Type = Type.FormColumn
}
//-------------------------------------------
// FormTemplate
//-------------------------------------------
...
//- Grid
//-------------------------------------------
//- Grid has direct Div tag children with class="gridrow". GridRow has direct children "GridCell".
//-------------------------------------------
Div.Extensions.Add(new Extension("grid", Grid));
Type.Grid = "Grid";
Type.GridRow = "GridRow";
Type.GridCell = "GridCell";
Type.GridMeasure = "GridColumns";

$Grid = $GridRow = $GridCell = $Anchor;

Grid.ResizeDelay = 200;
function Grid(e) {
    if (this.DOM === undefined) {
        return Grid.Extend(Grid.Inherit(new Div(e).ClassName("grid")));
    }
    ...
    $.Rows = [];
    $.__Calculating = false;
    var c = e.children, l = c.length, v;
    for (var i = 0; i < l; i++) {
        v = c[i].$;
        $.Rows.Add(GridRow.Inherit(GridRow.Extend(v)));
    }

    $.__Resize = new Timer(Grid.ResizeDelay, false);

    var d = new Delegate($.__CalculateRows, $ );
    $.__Resize.OnTrigger.Add(d);
    Client.OnResize.Add(new Delegate($.__Resize.Stop, [$.__Resize]));
    Client.OnResize.Add(new Delegate($.__Resize.Start, [$.__Resize]));
    Page.OnLoad.Add(d);

    $.__NativeWidth = Element.prototype.Width;
    $.__NativeHeight = Element.prototype.Height;
    $.Width = Grid.prototype.Width;
    $.Height = Grid.prototype.Height;
    $.Type = Type.Grid;
    $.Style(Style.BoxSizing, "border-box");
    $.__PrevCWidth = Client.Width();
    $.__PrevCHeight = Client.Height();
}

Grid.prototype.Width = function (w) {
    if (w === undefined) {
        return this.__NativeWidth(w);
    }

    this.__NativeWidth(w);
    this.__CalculateRows();
}

Grid.prototype.Height = function (h) {
    if (h === undefined) {
        return this.__NativeHeight(h);
    }

    this.__NativeHeight(h);
    this.__CalculateRows();
}

Grid.prototype.__CalculateRows = function () {
    ...
    for (var i = 0; i < l; i++) {
        v = r[i];
        switch (v.__Height.ColumnsType) {
            case GridColumnsType.Static:
                static.Add(v);
                break;

            case GridColumnsType.Relative:
                relative.Add(v);
                break;

            case GridColumnsType.Liquid:
                liquid.Add(v);
                break;
        }
    }

    var fullH = totalH = this.Height();
    var h;
    l = static.length;
    this.Collapsed(true);
    for (i = 0; i < l; i++) {
    ...
    for (i = 0; i < l; i++) {
        v = relative[i];
        h = fullH * (v.__Height.Value / 100)
        v.__NativeHeight(h);
        totalH -= h;
    };

    var rest = totalH / liquid.length;
    l = liquid.length;
    for (i = 0; i < l; i++) {
        v = liquid[i];
        v.__NativeHeight(rest);
    }

    this.Collapsed(false);
    l = r.length;
    for (i = 0; i < l; i++) {
        r[i].__CalculateCells();
    }
...
function GridRow(e) {
    if (this.DOM === undefined) {
        return GridRow.Inherit(GridRow.Extend(new Div(e).ClassName("row")));
    }

    var $ = this;
    e = $.DOM;
    $.__Height = new GridMeasure("*");
    $.Height = GridRow.prototype.Height;
    $.__Calculating = false;
    var a = e.getAttribute("gridheight");
    if (a) {
        $.__Height = new GridMeasure(a);
    }
    ...
    for (var i = 0; i < l; i++) {
        v = c[i].$;
        $.Cells.Add(GridCell.Inherit(GridCell.Extend(v)));
    }
...
GridRow.prototype.Height = function (h) {
    if (h === undefined) {
        return this.__NativeHeight(h);
    }

    this.__Height = new GridMeasure(h);
...
GridRow.prototype.__CalculateCells = function () {
    this.__Calculating = true;
    var static = [];
    var relative = [];
    var liquid = [];
    ...
    for (i = 0; i < l; i++) {
        v = relative[i];
        w = fullW * (v.__Width.Value / 100)
        v.__NativeWidth(w);
        totalW -= w;
    }

    var rest = totalW / liquid.length;
    l = liquid.length;
    for (i = 0; i < l; i++) {
        v = liquid[i];
        v.__NativeWidth(rest);
    }
...
function GridCell(e) {
    if (this.DOM === undefined) {
        return GridCell.Inherit(GridCell.Extend(new Div(e).ClassName("cell")));
    }

    var $ = this;
    e = $.DOM;
    $.__Width = new GridMeasure("*");
    var a = e.getAttribute("gridwidth");
    if (a) {
        $.__Width = new GridMeasure(a);
    }
    ...
    $.__NativeWidth = Element.prototype.Width;
    $.Width = GridCell.prototype.Width;
    $.Type = Type.GridCell;
    $.Height("100%");
    $.Style(Style.BoxSizing, "border-box");
}

GridCell.prototype.Width = function (w) {
    if (w === undefined) {
        return this.__NativeWidth(w);
    }

    this.__Width = new GridMeasure(w);
    var e = this.DOM.parentNode.$;
    if (!e.__Calculating) {
        e.__CalculateCells();
    }
...
function GridMeasure(value) {
    this.Value = null;
    this.ColumnsType = GridColumnsType.Liquid;
    this.Type = Type.GridMeasure;

    switch (value.Type) {
        case Type.String:
            if (value.EndsWith("%")) {
                this.Value = value.Remove("%").ToInt();
                this.ColumnsType = GridColumnsType.Relative;
            }
            else if (value == "*") {
                this.Value = null;
                this.ColumnsType = GridColumnsType.Liquid;
                ...
                this.ColumnsType = GridColumnsType.Static;
            }
            break;
...
function SuggestionBox(e) {
    ...
    $.__Display = new Div().Position("absolute").Collapsed(true);
    $.__Class = e.getAttribute("displayclass") || "";
    $.__FocusClass = (e.getAttribute("focusclass") || "");
    $.__Source = e.getAttribute("source") || null;
    $.__MinLength = (e.getAttribute("minlength") || "3");
    ...
    $.__Display.Style("overflow-y", "auto");
    $.__Display.Style("overflow-x", "hidden");
    $.Data = new Array(20);
    $.DisplayClass($.__Class);
    var d = new Delegate($.__HandleKeyDown, $, [Delegate.Event])
    $.__Display.OnKeyDown.Add(d);
    $.__CheckCache();
...
SuggestionBox.prototype.__HandleKeyDown = function (e) {
    ...
    switch (e.Key) {
        ...
        case Keys.DownArrow:
            if (this.__Display.Collapsed()) {
                this.ShowSuggestions();
            }
            this.__HighlightNext();
            break;
...
SuggestionBox.prototype.__HighlightNext = function () {
    if (this.__Display.Collapsed()) { return; }
    if (this.__CurrentAnchor) {
    ...
    if (this.__Display.ElementClipped(this.__CurrentAnchor)) {
        this.__Display.VerticalScrollToElement(this.__CurrentAnchor);
    }
};

SuggestionBox.prototype.__HighlightPrevious = function () {
    if (this.__Display.Collapsed()) { return; }
    if (this.__CurrentAnchor) {
        if (this.__FocusClass) {
            this.__CurrentAnchor.Classes.Remove(this.__FocusClass);
...
SuggestionBox.prototype.DisplayClass = function (c) {
    if (c === undefined) {
        return this.__Display.ClassName();
    }

    this.__Display.ClassName(c);
    if (!c) {
        this.__Display.Style(Style.Border, "1px solid #000000").BackgroundColor(Colors.White).Height(200);
    }
    else {
        this.__Display.Style(Style.Border, "").BackgroundColor(Colors.Empty).Style(Style.Padding, "");
    }
    return this;
}
...
SuggestionBox.prototype.ShowSuggestions = function () {
    if (!this.__StopShow) {
        if (this.Data.length > 0) {
            this.__Display.X(this.X()).Y(this.Y() + this.LayoutHeight());
            this.__Display.Width("").Collapsed(false).Width(this.__Display.LayoutWidth() + 17);
        }
    }
...
SuggestionBox.prototype.HideSuggestions = function (f) {
    if (f === undefined) {
        if (this.__Display.IsMouseOver) {
            return;
        }
    }

    this.__Display.Collapsed(true);
    return this;
};
...
//- Template
//-------------------------------------------
//- Templtaes cannot be dynamically created.
...
Template.prototype.Bind = function (obj) {
    ...
    if (!this.Template) {
        ...
        this.__nodeValues = [];
        var k = this.__all.length;
        for (var j = 0; j < k; j++) {
            this.__nodeValues.push(this.__all[j].nodeValue);
        }
    }

    var reg = /\{[A-Za-z\.]*}/i, property, a, b, k, v, w, x, y, l = this.__all.length;
    for (var i = 0; i < l; i++) {
...
function InlineTip(e) {
    if (this.DOM === undefined) {
        return InlineTip.Inherit(InlineTip.Extend(new Span(e).ClassName("tip")));
    }
}