...
function Menu(e) {
    ...
    $.__Effect = null;
    $.__SafeClip = e.getAttribute("safeclip");
    if (!$.__SafeClip) {
    ...
    if ($.IsRoot) {
        $.__EffectTmp = e.getAttribute("effect") || "none";
    }
    ...
    if (!$.IsRoot) {
        ...
        $.Style(Style.ZIndex, NinJaVisuals.ZIndex++)
    }
    else if (isCtx) {
        $.NaturalDisplay = "block";
        $.__Effect = new Effect($, $.Root.__EffectTmp, 0, 0, Speed.Fastest, Smoothness.Normal);
        Page.OnClick.Add($.__HideDelegate3);
        $.Style(Style.ZIndex, NinJaVisuals.ZIndex++)
    }
    else {
    ...
    if (!isCtx) {
        $.__HideTimer = new Timer(500, false);
        $.__ShowTimer = new Timer(500, false);
...
Menu.prototype.Show = function () {
    ...
    if (this.OnOpening.Fire(new EventArgs(this))) {
        ...
        if (this.__SafeClip) {
            if (Client.ElementClipped(this, Side.Right)) {
                Client.SnapTo(this, Side.Right);
            }
            if (Client.ElementClipped(this, Side.Bottom)) {
                Client.SnapTo(this, Side.Bottom);
            }
        }

        this.Collapsed(true).Visible(true);
        this.__Effect.Play();
...
ContextMenu.prototype.Bind =
Menu.prototype.Bind = function (menuTree) {
    this.Items.Clear();
}
...
function MenuItem(e) {
    ...
    if ($.__Href) {
        $.OnClick.Add(new Delegate(Page.Location, Page, [$.__Href]));
    }
}
...
MenuItem.prototype.Submenu = function (s) {
    ...
    if (s) {
        ...
        s.IsRoot = false;
        s.Root = this.ParentMenu.Root;
        s.__Effect = new Effect(s, s.Root.__EffectTmp, 0, 0, Speed.Fastest, Smoothness.Normal);
