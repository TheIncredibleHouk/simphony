...
///<reference path="~/scripts/NinJaControls.js" />
...
function GridView(e) {
    ...
    var $ = this;
    e = $.DOM;
    $.__Display = (e.getAttribute("display") || "10").ToInt();
    ...
    for (i = 0; i < l; i++) {
        v = c[i];
        $.Columns.Add(GridViewColumn.Inherit(GridViewColumn.Extend(v), g[i]));
    }
    ...
    $.__Selectable = (e.getAttribute("selectable") || "true").ToBoolean();
    ...
    $.__RowHeight = (e.getAttribute("rowheight") || "");
    $.__Loading = false;
    ...
    $.Holder.AddElement($);
    $.Container.AddElements([$.ScrollBar, $.Holder]);
    ...
    e.__Sort = null;
    e.Type = Type.GridView;

    if (Client.Storage.Local) {
        i = Client.Storage.Local.GetItem(Page.Location().pathname + ":" + this.DOM.id);
        if (i) {
            this.Sort(i.s, i.o);
        }
    }
...
GridView.prototype.Sort = function (e, o) {
    ...
    if (this.SortIcon.DOM.parentNode) {
        this.SortIcon.DOM.parentNode.$.ClassName("");
    }
    ...
    if (!p) {
        ...
        this.__Sort = null;
        if (this.SortIcon.DOM.parentNode) {
            this.SortIcon.DOM.parentNode.$.ReSnapTo(this.SortIcon).ClassName("");
        }
        return;
    }
    ...
    if (this.SortIcon.DOM.parentNode) {
        this.SortIcon.DOM.parentNode.$.ClassName(this.__SortClass || (this.__Order == SortOrder.Ascending ? this.__ASortClass : this.__DSortClass) || "");
    }
    this.__Sort = p;
    this.Data.Sort(p, this.__Order);
    this.Refresh();
    if (Client.Storage.Local) {
        Client.Storage.Local.SetItem(Page.Location().pathname + ":" + this.DOM.id, { s: this.__Sort, o: this.__Order });
    }
    return this;
};
...
GridView.prototype.RefreshData = function () {
    ...
    if (this.Data.length <= this.__Display) {
        ...
        this.ScrollBar.MaxValue(this.Data.length - this.__Display);
        this.Refresh();
    }
};
...
GridView.prototype.ScrollToItem = function (item) {
    if (item) {
        var index = this.Data.IndexOf(item), a = this.ScrollBar.Value(), b = this.__Display, c = b + a;
        if (index >= a && index < c) {
            return;
        }
        else if (index >= c) {
...
GridView.prototype.HandleClick = function (e) {
    ...
    if (e.Sender.Tag) {
        if (this.SelectedData.Contains(e.Sender.Tag)) { return; }
        this.SelectedData.BeginUpdate();
        this.SelectedData.Clear();
        this.SelectedData.Add(e.Sender.Tag);
        this.__LastIndex = e.Sender.Index;
...
GridView.prototype.HandleKey = function (e) {
    var i = this.__LastIndex, d = this.Data, sd = this.SelectedData;
    ...
    switch (e.Key) {
        case Keys.DownArrow:
            ...
            if (false) {
                ...
                sd.Clear();
                sd.Add(d[i]);
                if (this.__LastRow == this.__Display) {
        ...
        case Keys.PageUp:
            ...
            if (false) {
                ...
                sd.Clear();
                sd.Add(d[i]);
                this.ScrollBar.Value(this.ScrollBar.Value() - this.__Display);
                sd.Updating = false;
            }
            sd.EndUpdate();
            break;
...
GridView.prototype.RefreshWidth = function () {
    ...
    var total = this.Container.LayoutWidth() - this.ScrollBar.LayoutWidth();
    ...
    for (i = 0; i < l; i++) {
        ...
        v = w.__Width;
        switch (v.Type) {
            case GridColumnSizeType.Static:
                w.Width(w.Value);
                total -= w.Value;
                break;

            case GridColumnSizeType.Relative:
                u = total * v.Value;
                w.Width(u);
                total -= u;
                break;

            case GridColumnSizeType.Liquid:
                lq.Add(w);
                break;
        }
    }
...
function GridViewColumn(c) {
    if (this.DOM === undefined) {
        return GridViewColumn.Inherit(GridViewColumn.Extend(new Header()));
    }
    ...
    $.__HTML = (e.getAttribute("html") || "false").ToBoolean();
    $.__Sort = (e.getAttribute("sortable") || "true").ToBoolean();
    $.__Width = new GridColumnSize(e.getAttribute("width") || "*");
    $.__Display = e.getAttribute("display") || "";
...
GridViewColumn.prototype.__Premutate = function (d) {
    return { __T: String.BindFormat(this.__Display, d), __D: NinJa.EvalBind(this.__Data, d) };
};
...
GridViewRow.prototype.Bind = function (d) {
    ...
    for (i = 0; i < l; i++) {
        ...
        w = v.__Col;
        ev.Cancel = false;
        if (!NullOrUndefined(d) && w.OnColumnBound.length > 0) {
            ev.Column = w;
            ev.Item = d;
            ev.Sender = v;
        ...
        if (!d || ev.Cancel) {
            ...
            switch (w.__Type) {
                ...
                case GridViewColumnType.Anchor:
                    v.__A.Text(t).Href(u).Collapsed(false);
                    break;
...
var GridColumnSizeType = {
    Static: 0,
    Relative: 1,
    Liquid: 2
};

function GridColumnSize(value) {
    this.Value = null;
    this.ColumnsType = GridColumnSizeType.Liquid;
    this.Type = Type.GridColumnSize;

    switch (value.Type) {
        case Type.String:
            if (value.EndsWith("%")) {
                this.Value = value.Remove("%").ToFloat() / 100;
                this.ColumnsType = GridColumnSizeType.Relative;
            }
            else if (value == "*") {
                this.Value = null;
                this.ColumnsType = GridColumnSizeType.Liquid;
                ...
                this.ColumnsType = GridColumnSizeType.Static;
            }
            break;
