...
function Tab(e) {
    ...
    e = $.DOM;
	$.__SelectedItem = null;
	$.Container = new Div().ClassName("container").Height("100%");
	$.Header = new Div().ClassName("headerpanel");
...
Tab.prototype.SelectedItem = function (t) {
    ...
    if (this.__SelectedItem) {
        this.__SelectedItem.Header.Classes.Remove("selected");
        this.__SelectedItem.Header.Classes.Add("header");
        this.__SelectedItem.Collapsed(true);
    }

    this.__SelectedItem = t;
    this.__SelectedItem.Header.Classes.Remove("header");
    this.__SelectedItem.Header.Classes.Add("selected");
    this.__SelectedItem.Collapsed(false);
    if (this.OnSelectionChanged.length > 0) {
        this.OnSelectionChanged.Fire(new EventArgs(this, this.__SelectedItem));
    }
    return this;
};
...
Tab.prototype.TabLocation = function (l) {
    ...
    this.ReSnapTos([this.Header, this.Container]);
    var s;
    switch (l) {
        case "top":
            this.Header.Style(Style.Display, "block");
            this.Container.Style(Style.Display, "block").Style(Style.Float, "");
            this.AddElements([this.Header, this.Container]).NaturalDisplay = "block";
            s = "inline-block";
            break;
        ...
        case "bottom":
            this.Header.Style(Style.Display, "block");
            this.Container.Style(Style.Display, "block").Style(Style.Float, "");
            this.AddElements([this.Container, this.Header]).NaturalDisplay = "block";
            s = "inline-block";
            break;

        case "left":
            this.Header.Style(Style.Display, "inline-block");
            this.Container.Style(Style.Display, "inline-block").NaturalDisplay = "inline-block";
...
function TabItem(e) {
    ...
    e = $.DOM;
    $.Style("zoom", "1");
    $.Header = $.ElementsByClassName("header")[0];
    if(!$.Header){
...
function TabHeader(e){
    if (this.DOM === undefined) {
        return TabHeader.Inherit(TabHeader.Extend(new Div(e).ClassName("header")));
    }
}
