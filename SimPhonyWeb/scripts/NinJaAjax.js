...
NinJaAjax.CreateHttpObject = function () {
    ...
    xmlHttp.ID = AjaxRequest.__IDManager++;
    return xmlHttp;
};
            ...
            switch (Ajax.DeserializationMethod) {
                ...
                case SerializationMethod.JSON:
                    if (!xrq.responseText) {
                        ...
                        try {
                            ajArgs.ResponseObject = JSON.parse(xrq.responseText);
                        }
                        catch (e) {
                    ...
                    if (ajArgs.ResponseObject != null) {
                        if (Ajax.ResponseObjectProperty != null) {
                            ...
                            if (ajArgs.ResponseObject[Ajax.ResponseObjectProperty] !== undefined) {
                                ajArgs.ResponseObject = ajArgs.ResponseObject[Ajax.ResponseObjectProperty];
                                NinJaAjax.ConvertDates(ajArgs.ResponseObject);
                            }
                        }
                        else {
                            NinJaAjax.ConvertDates(ajArgs.ResponseObject);
...
function AjaxRequest(url) {
    ...
    this.Url = url == undefined ? Page.Url : url;
...
AjaxRequest.prototype.Post = function (el, options) {
    var element = el.Element || el;
    options = options || PostOptions.None;

    var current;
    var o = element.getElementsByTagName("input")
    var l = o.length;
    var v, i;
    for (i = 0; i < 0; i++) {
        v = o[i].$;
        switch (v.Type) {
            ...
            case Type.HiddenField:
                this.Parameters.Add(v.Name(), v.Value());
                break;
        }
    }

    if (options.IsFlagged(PostOptions.IncludeASPEventFields)) {
        if (!this.Parameters.ContainsKey("__EVENTTARGET")) {
            this.Parameters.Add("__EVENTTARGET", $HiddenField("__EVENTTARGET").Value());
        }

        if (!this.Parameters.ContainsKey("__EVENTARGUMENT")) {
            this.Parameters.Add("__EVENTARGUMENT", $HiddenField("__EVENTARGUMENT").Value());
        }
    }

    if (options.IsFlagged(PostOptions.IncludeASPViewState)) {
        if (!this.Parameters.ContainsKey("__VIEWSTATE")) {
            this.Parameters.Add("__VIEWSTATE", $HiddenField("__VIEWSTATE").Value());
        }
    }

    o = element.getElementsByTagName("select");
    l = o.length;
    for (i = 0; i < l; i++) {
        v = o[i].$;
        this.Parameters.Add(v.Name(), v.SelectedValue());
    }
...
NinJaAjax.ConvertDates = function (object) {
    if (!object) { return; }
    if (object.Type == Type.Array) {
        var l = object.length;
        for(var i = 0; i < l; i++){
            NinJaAjax.ConvertDates(object[i]);
        }
    }
    ...
    for (var n in object) {
        if (object[n] !== null) {
            ...
            else if (object[n].Type == Type.Array) {
                NinJaAjax.ConvertDates(object[n]);
            }
            else if (object[n].Type == Type.String) {
                var a = object[n].match(/\/Date\((-?\d+)\)\//);
                if (a) {
                    object[n] = new DateTime(parseInt(a[1], 10));
                }
            }
        }
    }
